...
Процедура Выпустить(Знач ПутьКФайлуИлиКаталогу) Экспорт
    ...
    НастройкаВыпуска.ЗагрузитьИзФайла(ПутьКФайлуНастройки);

    Контекст = Новый КонтекстВыпуска(НастройкаВыпуска);
    Контекст.РабочийКаталог = Новый Файл(ПутьКФайлуНастройки).Путь;
...
Процедура ВыполнитьСборкиВыпуска(Знач Контекст)
    ...
    Для Каждого ЭлементСборки Из МассивСборки Цикл
        ...
        Лог.Отладка("Выполняю шаг: %1", Строка(ЭлементСборки));
...
Лог = Логирование.ПолучитьЛог("oscript.lib.orca.runner");
